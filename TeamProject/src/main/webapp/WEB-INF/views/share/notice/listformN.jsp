...
<body>
	...
	onclick="location.href='writeformN'">글쓰기</button>
	</h2>
	<c:choose>
	<c:when test="${articleList!=null && pageInfo.listCount>0 }">
		<section id="listFormN">
		<table>
			<tr id="tr_top">
				<td>번호</td>
				<td>제목</td>
			...
			<c:forEach var="article" items="${articleList }">
				...
				<a href="./boarddetailN?board_num=${article.board_num}&page=${pageInfo.page}"><!-- 링크 -->
		...
		<section id="pageList">   <!-- 페이지 목록 --><!-- if else 랑 같음.  페이지가 1보다 작거나 같으면 이전페이지로 -->
			<c:choose>
				...
				<c:otherwise>
					<a href="boardlistN?page=${pageInfo.page-1}">[이전]</a>&nbsp;
				</c:otherwise>  
			</c:choose>   
			...
			<c:choose>
				...
				<c:otherwise>
					<a href="boardlistN?page=${pageInfo.page+1}">[다음]</a>  
				</c:otherwise> 
			</c:choose>
